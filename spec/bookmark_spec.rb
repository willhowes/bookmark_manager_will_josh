...
describe Bookmark do
  ...
  describe '.update' do
    it 'updates the bookmark with the given data' do
      bookmark = Bookmark.create(title: 'Makers Academy', url: 'www.makersacademy.com')
      updated_bookmark = Bookmark.update(id: bookmark.id, url: 'www.snakersacademy.com', title: 'Snakers Academy')

      expect(updated_bookmark).to be_a Bookmark
      expect(updated_bookmark.id).to eq bookmark.id
      expect(updated_bookmark.title).to eq 'Snakers Academy'
      expect(updated_bookmark.url).to eq 'www.snakersacademy.com'
    end
  end
  ...
  describe '.find' do
    it 'returns the requested bookmark object' do
      ...
      expect(result.url).to eq 'www.makersacademy.com'
    end
  end
end
